<x-super-admin-layout>
    <x-slot name="header">{{ __('New FAQ') }}</x-slot>

    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg max-w-3xl">
        <div class="p-6">
            <form method="POST" action="{{ route('super-admin.faqs.store') }}">
                @csrf
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <select name="category" id="category" required
                            ...
                            @foreach($categories as $cat)
                                <option value="{{ $cat['slug'] }}" {{ (old('category', request('category')) === $cat['slug']) ? 'selected' : '' }}>
                                    {{ $cat['label'] }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="question" id="question" value="{{ old('question') }}" required maxlength="1000"
                               class="w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                               placeholder="e.g. What services do you offer for restaurants?">
                        @error('question')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="answer" id="answer" rows="5" required maxlength="5000"
                                  class="w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                                  placeholder="Full answer text...">{{ old('answer') }}</textarea>
                        @error('answer')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
